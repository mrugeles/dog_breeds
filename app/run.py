...
def predict(img_path):
    bottleneck_feature = bottleneck_feature = extract_Xception(path_to_tensor(img_path))
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            with graph.as_default():
                predictions = predict('static/'+file_path)[0]
                best_prediction = np.argmax(predictions)
                print(predictions)
                print(best_prediction)
                best_prediction_probability = predictions[best_prediction]
